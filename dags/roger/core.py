    ...
    def insert (self):
        ...
        if len(nodes) > 0:
            ...
            for x in nodes:
                ...
                """
                file_name_type_part = x.replace(bulk_path_root, '').split('.')[0].replace('~', '.')
                all_labels = file_name_type_part + ":" + ":".join([v.replace("biolink:", "biolink.") for v in self.biolink.toolkit.get_ancestors("biolink:" + file_name_type_part, reflexive=False, formatted=True )] )
